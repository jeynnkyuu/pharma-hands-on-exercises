...
```{r}
param_plot <- function(paramn=NULL){
  ...
  xLab <- quo_name(var$mapping$x)
  yLab <- quo_name(var$mapping$y)
  groupLab <- quo_name(var$mapping$group)
  
  study <- uniqueVal(var$data$STUDYID)
  measure <- uniqueVal(var$data$PARAM)
...
DiaBP_plot <- param_plot(2)
DiaBP_plot


temp_plot <- param_plot(6) 
temp_plot
```
...
pr <-  advs %>% 
    filter(PARAMN == 3) %>% 
    filter(SAFFL == "Y" & VISITNUM > 3 & AGE < 60)

plot1 <- ggplot(data = pr, mapping = aes(x = ADY, y = AVAL, group = USUBJID, color = USUBJID)) +
    geom_line()
plot1

xLab <- quo_name(plot1$mapping$x)
yLab <- quo_name(plot1$mapping$y)
groupLab <- quo_name(plot1$mapping$group)
  
study <- uniqueVal(plot1$data$STUDYID)
measure <- "Pulse Rate (Beats/Min)" 
  #uniqueVal(plot1$data$PARAM)

plot2 <- plot1 +
    labs(title = glue::glue("Study {study}"),
          subtitle = glue::glue("Plot of {measure} by {groupLab}"),
          x = "Day within study period",
          y = glue::glue("{measure}"),
          caption = paste("Plot Created on: ", Sys.Date())) +
          theme_minimal() +
          facet_wrap(facets = "TRTA")
