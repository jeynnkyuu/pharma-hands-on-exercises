...
stat <- function(var){
  adsl_eff %>%
    group_by(TRT01AN, TRT01A, SEX) %>%
    summarize(mean = (mean( {{var}} ) %>% round(digits = 1) %>% format(nsmall=1))  ,
              sd = (sd( {{var}} ) %>% round(digits = 1) %>% format(nsmall = 1)), 
              med = (median( {{var}} )),              
              min = (min( {{var}} )),  
              max = (max( {{var}} )), 
              n = (n())) %>% 
...
section <- c("Age by years", "Weight at baseline", "Height at baseline")

all_stat <- rbind(section[1], stat(AGE), section[2], stat(WEIGHTBL), section[3], stat(HEIGHTBL))

all_stat[1, 2:ncol(all_stat)] <- ""
all_stat[7, 2:ncol(all_stat)] <- ""
all_stat[13, 2:ncol(all_stat)] <- ""
              
all_stat_fin <- data.frame(all_stat)
all_stat_fin
```
