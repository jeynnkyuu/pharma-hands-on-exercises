...
test_that("wrangle_data() output is correct", {
  ...
  # names(adsl_data) has length 6, not length 5.
...
test_that("calculate_small_n_cnt() output has expected dim, columns, and types", {
   ...
   expect_equal(dim(calculate_small_n_cnt(test_data)), 
               c(length(unique(calculate_small_n_cnt(test_data)$TRT01A)), 2)*c(length(unique(calculate_small_n_cnt(test_data)$SEX)), 2))
})
```
...
```{r}
## Ensure all variables are present in adsl_counts and it has a correct type

test_that("Output data has expected column names and types", {
...
```{r}
## Expected dimension of the output is 4 by 3 since we have two treatments and sex (which make it 4 rows) and 3 expected variables present in the output
test_that("Dimension is correct", {expect_equal(dim(adsl_test_data), c(4,3))})

```

```{r}
## No missing percentages for each category

test_that("Compelete percatages for each Treatment by Sex", {expect_success(expect_equal(sum(is.na(adsl_test_data$perc)), 0))})

```

```{r}
## There are two treatments in the output
test_that("All treatments are present", {expect_success(expect_equal(unique(adsl_test_data$TRT01A), c("Treatment A", "Treatment B")))})

```

```{r}
## Sex is spelled correctly
test_that("All Sex are present", {expect_success(expect_equal(unique(adsl_test_data$SEX), c("Female", "Male")))})

```
 
```{r}
## Percentages is not greater than 100%
test_that("No percentages is greater than 100%", {expect_success(expect_equal(sum(adsl_test_data$perc<100), length(adsl_test_data$perc)))})
