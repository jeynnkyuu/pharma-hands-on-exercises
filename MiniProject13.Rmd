...
summ_by_grp <- function(data, group_var) {
  group_var <- rlang::ensym(group_var)  # capture the variable name
  ...
  small_n_cnt <- data %>%
    group_by(TRT01AN, TRT01A, !!group_var) %>%
    count(name = "n")
  ...
  result <- small_n_cnt %>%
    ...
    ungroup() %>%
    select(TRT01A, !!group_var, npct) %>%
    pivot_wider(names_from = TRT01A, values_from = npct)
  
  return(result)
}

summ_by_grp(adsl_saf, SEX)
summ_by_grp(adsl_saf, AGEGR1)
summ_by_grp(adsl_saf, RACE)
