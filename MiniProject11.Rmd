...
mySummary <- function(myData, ...){
  myData %>%
    ...
    summarise(mean = round(mean(AGE), ... ), .groups = 'keep')
}
...
```{r}
### Apply the embrace and dynamic dots concepts you learned in this mini project to the `adsl_counts` function that you developed in Mini-Project 7
adsl_counts <- function(adsl_inds, var, ...) {
...
inFile2 <- "https://github.com/phuse-org/phuse-scripts/raw/master/data/adam/cdisc/adsl.xpt"

adsl <- import(inFile2) 



adsl_counts(adsl, SEX, "M" = "Male", "F" = "Female")
### Third argument (...) should always be present 
# This will prompt to error
# adsl_counts(adsl, RACE)
# Use these instead
adsl_counts(adsl, RACE, " " = " ")
adsl_counts(adsl, RACE, "ASIAN" = "Asian", "WHITE" = "White")
```
