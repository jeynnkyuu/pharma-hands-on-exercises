...
```{r}
### Challenge: What Went wrong here
# THERAPHY1 is not a variable, hence, we cannot use it as a grouping variable
# Use TRTA instead to properly group the values by color
ggplot(data = BICARB,
       mapping = aes(x = ADY, y = AVAL)) +
  geom_point(aes(colour = TRTA))
```


```{r}
### Create a more minimal reprex that will help someone else understand and help you fix the problem.
# Create data using tribble() function
library(tidyverse)
library(haven)
